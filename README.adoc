...
Installs the newest fish shell on Ubuntu via the fish PPA repository, allows fish as a login, and configures the fish shell according to my preferences for the given _user_, which defaults to the user running Ansible.
It also configures fish as the login shell for the user.

== Variables

The only variable is `user`.
This is the user for whom fish is configured.
...
// todo Use -c local for local connection?
Command-Line::
+
[source,sh]
----
➜ ansible localhost -m include_role -a name=ansible_fish
----
